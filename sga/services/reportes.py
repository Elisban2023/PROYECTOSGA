...
def build_reporte_resumen():
    return {
        ...
        "academico": {
            "grados_activos": Grado.objects.filter(estado=EstadoRegistro.ACTIVO).count(),
            "secciones_activas": Seccion.objects.filter(estado=EstadoRegistro.ACTIVO).count(),
            "cursos_activos": Curso.objects.filter(estado=EstadoRegistro.ACTIVO).count(),
            "asignaciones_activas": AsignacionCurso.objects.filter(estado=EstadoGeneral.ACTIVO).count(),
